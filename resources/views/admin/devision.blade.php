...
@section('title', 'Data Divisi - Alena Mandiri Group')
            ...
            <div class="content-wrapper">
                ...
                <div class="table-card">
                    <div class="table-header">
                        <h3 class="table-title"><i class="bi bi-diagram-3 me-2"></i>Data Divisi</h3>

                        <div class="table-actions">
                            {{-- SEARCH --}}
                            <div class="search-wrapper">
                                <i class="search-icon bi bi-search"></i>
                                <input type="text" class="search-input" id="searchInput" placeholder="Cari divisi...">
                            </div>
                        ...
                        <table class="table" id="divisionTable">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-left">
                                        <i class="bi bi-diagram-3 me-1"></i>Nama Divisi
                                        <i class="bi bi-arrow-down-up sort-icon"></i>
                                    </th>
                                    <th class="text-center">
                            ...
                            <tbody>
                                ...
                                <tr data-id="{{ $division->id }}" data-name="{{ $division->name }}">
                                    <td class="text-center">{{ $index + 1 }}</td>
                                    <td class="text-left">{{ $division->name }}</td>
                                    <td class="text-center">
                                        <div class="action-btns">
                                            <div class="action-btn edit" onclick="openModal('edit', {{ $division->id }}, '{{ $division->name }}')">
                                                <i class="bi bi-pencil"></i>
                                            </div>
                                            <div class="action-btn delete" onclick="openModal('hapus', {{ $division->id }}, '{{ $division->name }}')">
...
<div class="modal" id="divisiModal">
    <div class="modal-content">
        <form id="divisiForm" method="POST" action="">
            @csrf
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="name" id="namaDivisi" placeholder="Masukkan nama divisi" value="{{ old('name') }}" required>
                </div>
            </div>
        ...
        <form id="hapusForm" method="POST" action="">
            ...
            <div class="modal-body" style="text-align: center; padding: 30px 24px;">
                ...
                <p style="color: var(--text-secondary); font-size: 14px;" id="hapusText">
                    Divisi <strong></strong> akan dihapus permanen.
                </p>
            </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function loadAllRows() {
            allRows = [];
            $('#divisionTable tbody tr').each(function() {
                const $row = $(this);
        ...
        function renderTable() {
            ...
            $('#divisionTable tbody tr').hide();

            if (pageRows.length === 0) {
                if ($('#divisionTable tbody .no-data-row').length === 0) {
                    $('#divisionTable tbody').html('<tr class="no-data-row"><td colspan="3" class="text-center no-data"><i class="bi bi-diagram-3"></i><p>Tidak ada data ditemukan</p></td></tr>');
                } else {
                    $('#divisionTable tbody .no-data-row').show();
                }
            } else {
                $('#divisionTable tbody .no-data-row').remove();
        ...
        if (typeof TableHandler !== 'undefined') {
            TableHandler.init('divisionTable', {
    ...
    function openModal(type, id = null, divisiName = '') {
        if (type === 'hapus') {
            if (id && divisiName) {
                currentDivisiId = id;
                document.getElementById('hapusText').innerHTML = `Divisi <strong>${divisiName}</strong> akan dihapus permanen.`;
                document.getElementById('hapusForm').action = `{{ url('divisions') }}/${id}`;
            }
            document.getElementById('hapusModal').classList.add('active');
        } else {
            const modal = document.getElementById('divisiModal');
            const title = document.getElementById('modalTitle');
            const inputNama = document.getElementById('namaDivisi');
            ...
            if (type === 'edit' && id) {
                currentDivisiId = id;
                title.innerHTML = '<i class="bi bi-pencil me-2"></i>Edit Divisi';
                form.action = `{{ url('divisions') }}/${id}`;
                methodField.value = 'PUT';
                if (divisiName) {
                    inputNama.value = divisiName;
                }
            } else {
                title.innerHTML = '<i class="bi bi-plus-circle me-2"></i>Tambah Divisi';
    ...
    function closeModal() {
        document.getElementById('divisiModal').classList.remove('active');
    }
