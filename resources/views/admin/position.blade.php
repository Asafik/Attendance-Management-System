                    ...
                    <div class="table-header">
                        ...
                        <div class="table-actions">
                            ...
                            <div class="search-wrapper">
                                ...
                                <input type="text" class="search-input" id="searchInput" placeholder="Cari jabatan...">
                            </div>
                    ...
                    <div class="table-responsive">
                        <table class="table" id="positionTable">
                            ...
                            <tbody>
                                ...
                                <tr data-id="{{ $position->id }}" data-name="{{ $position->name }}">
                                    <td class="text-center">{{ $index + 1 }}</td>
                                    <td class="text-left">{{ $position->name }}</td>
                                    <td class="text-center">
                                        <div class="action-btns">
                                            <div class="action-btn edit" onclick="openModal('edit', {{ $position->id }}, '{{ $position->name }}')">
                                                <i class="bi bi-pencil"></i>
                                            </div>
                                            <div class="action-btn delete" onclick="openModal('hapus', {{ $position->id }}, '{{ $position->name }}')">
        ...
        <form id="positionForm" method="POST" action="">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="name" id="namaPosition" placeholder="Masukkan nama jabatan" value="{{ old('name') }}" required>
                </div>
            </div>
        ...
        <form id="hapusForm" method="POST" action="">
            ...
            <div class="modal-body" style="text-align: center; padding: 30px 24px;">
                ...
                <p style="color: var(--text-secondary); font-size: 14px;" id="hapusText">
                    Jabatan <strong></strong> akan dihapus permanen.
                </p>
            </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function renderTable() {
            ...
            $('#positionTable tbody tr').hide();

            if (pageRows.length === 0) {
                if ($('#positionTable tbody .no-data-row').length === 0) {
                    $('#positionTable tbody').html('<tr class="no-data-row"><td colspan="3" class="text-center no-data"><i class="bi bi-briefcase"></i><p>Tidak ada data ditemukan</p></td></tr>');
                } else {
                    $('#positionTable tbody .no-data-row').show();
                }
            } else {
                $('#positionTable tbody .no-data-row').remove();
        ...
        if (typeof TableHandler !== 'undefined') {
            TableHandler.init('positionTable', {
    ...
    function openModal(type, id = null, positionName = '') {
        if (type === 'hapus') {
            if (id && positionName) {
                currentPositionId = id;
                document.getElementById('hapusText').innerHTML = `Jabatan <strong>${positionName}</strong> akan dihapus permanen.`;
                document.getElementById('hapusForm').action = `{{ url('positions') }}/${id}`;
            }
            document.getElementById('hapusModal').classList.add('active');
        } else {
            const modal = document.getElementById('positionModal');
            const title = document.getElementById('modalTitle');
            const inputNama = document.getElementById('namaPosition');
