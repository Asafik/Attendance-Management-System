                ...
                <div class="error-card" style="background-color: var(--bg-card); border-radius: 30px; padding: 50px 40px; box-shadow: 0 20px 40px rgba(0,0,0,0.1), 0 4px 12px rgba(22,163,74,0.2); border: 1px solid var(--border-color); position: relative; overflow: hidden;">
                    ...
                    <div class="text-center position-relative" style="z-index: 2;">
                        <div class="error-number-container mb-4">
                            ...
                            <span class="error-digit">9</span>
                        </div>

                        {{-- ANIMASI TEKS --}}
                        <h2 class="error-title mb-3">
                            <span class="fade-in">Sesi</span>
                            <span class="fade-in delay-1">Telah</span>
                            <span class="fade-in delay-2">Berakhir</span>
                        </h2>
                        ...
                        <div class="error-description-wrapper mb-4">
                            <p class="error-description slide-up">
                                ...
                                @if(session('error_type') == 'multiple_login')
                                    Akun Anda telah login di perangkat lain. Anda otomatis logout.
                                @else
                        ...
                        <div class="info-card mb-4 pulse-animation" style="background: linear-gradient(145deg, var(--accent-soft) 0%, rgba(22,163,74,0.05) 100%); border-radius: 20px; padding: 20px; border: 1px solid var(--accent-color);">
                            <div class="d-flex align-items-center gap-3">
                                <div style="width: 50px; height: 50px; background: var(--accent-color); border-radius: 15px; display: flex; align-items: center; justify-content: center;">
                                    <i class="bi bi-shield-check" style="font-size: 24px; color: white;"></i>
                                </div>
                                <div class="text-start">
                                    <h6 style="color: var(--text-primary); margin: 0; font-weight: 600;">Keamanan Akun</h6>
                                    <p style="color: var(--text-secondary); margin: 0; font-size: 13px;">
                                        @if(session('error_type') == 'multiple_login')
                                            Anda logout karena login di perangkat lain. Jika bukan Anda, segera hubungi administrator.
                                        @else
                                            Demi keamanan, sesi akan berakhir otomatis setelah periode tidak aktif.
                                        @endif
                                    </p>
                                </div>
                            </div>
                        ...
                        {{-- BUTTON LOGIN KEMBALI (SATU-SATUNYA) --}}
                        <div class="d-flex gap-3 justify-content-center flex-wrap">
                            <a href="{{ route('login') }}" class="btn-dashboard hover-animation" style="background: var(--accent-color); color: white; border: none; padding: 14px 35px; border-radius: 50px; font-weight: 500; display: inline-flex; align-items: center; gap: 10px; text-decoration: none; box-shadow: 0 8px 20px rgba(22,163,74,0.3);">
                        ...
                        {{-- ANIMASI LOADING DOTS & COUNTDOWN --}}
                        <div class="mt-4">
                            ...
                            <div id="countdown" style="color: var(--text-secondary); font-size: 14px;">
                                <i class="bi bi-clock-history me-1"></i>
                                <span id="countdownNumber">10</span> detik lagi redirect ke login...
                            </div>
                        </div>
                    </div>
                </div>
...
@push('scripts')
<script>
    // Auto redirect countdown (999 detik untuk cek UI)
    let seconds = 10;
