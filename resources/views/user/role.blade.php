...
@section('title', 'Data Role - Alena Mandiri Group')
            ...
            <div class="content-wrapper">
                ...
                <div class="table-card">
                    <div class="table-header">
                        <h3 class="table-title"><i class="bi bi-tags me-2"></i>Data Role</h3>
                    ...
                    <div class="table-responsive">
                        <table class="table" id="roleTable">
                            ...
                            <tbody>
                                ...
                                <tr data-id="{{ $role->id }}" data-name="{{ $role->name }}" data-display_name="{{ $role->display_name }}">
                                    <td class="text-center">{{ $index + 1 }}</td>
                                    <td class="text-left">{{ $role->name }}</td>
                                    <td class="text-left">{{ $role->display_name }}</td>
                                    <td class="text-center">
                                        <div class="action-btns">
                                            <div class="action-btn edit" onclick="openModal('edit', {{ $role->id }}, '{{ $role->name }}', '{{ $role->display_name }}')">
                                                <i class="bi bi-pencil"></i>
                                            </div>
                                            <div class="action-btn delete" onclick="openModal('hapus', {{ $role->id }}, '{{ $role->name }}')">
        ...
        <form id="roleForm" method="POST" action="">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="name" id="roleName" placeholder="Masukkan name (contoh: admin, super-admin)" value="{{ old('name') }}" required>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="display_name" id="displayName" placeholder="Masukkan display name (contoh: Administrator)" value="{{ old('display_name') }}" required>
                </div>
            </div>
        ...
        <form id="hapusForm" method="POST" action="">
            ...
            <div class="modal-body" style="text-align: center; padding: 30px 24px;">
                ...
                <p style="color: var(--text-secondary); font-size: 14px;" id="hapusText">
                    Role <strong></strong> akan dihapus permanen.
                </p>
            </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function renderTable() {
            ...
            if (pageRows.length === 0) {
                if ($('#roleTable tbody .no-data-row').length === 0) {
                    $('#roleTable tbody').html('<tr class="no-data-row"><td colspan="4" class="text-center no-data"><i class="bi bi-tags"></i><p>Tidak ada data ditemukan</p></td></tr>');
                } else {
                    $('#roleTable tbody .no-data-row').show();
                }
            } else {
                $('#roleTable tbody .no-data-row').remove();
    ...
    function openModal(type, id = null, name = '', displayName = '') {
        if (type === 'hapus') {
            if (id && name) {
                currentRoleId = id;
                document.getElementById('hapusText').innerHTML = `Role <strong>${displayName || name}</strong> akan dihapus permanen.`;
                document.getElementById('hapusForm').action = `{{ url('roles') }}/${id}`;
            }
            ...
            if (type === 'edit' && id) {
                currentRoleId = id;
                title.innerHTML = '<i class="bi bi-pencil me-2"></i>Edit Role';
                ...
                title.innerHTML = '<i class="bi bi-plus-circle me-2"></i>Tambah Role';
